      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
        ...
        <div className="lg:sticky lg:top-32 self-start space-y-8 sm:space-y-12">
          ...
          <div className="space-y-4 sm:space-y-8">
            <div className="flex items-center gap-4 sm:gap-6 p-4 sm:p-6 bg-slate-50 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-100 group transition-all duration-300 hover:shadow-lg hover:bg-white">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-amber-500 text-white rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg shadow-amber-500/20 group-hover:scale-110 transition-transform shrink-0">
              ...
              <div>
                <h4 className="font-black text-slate-900 uppercase text-[10px] tracking-widest mb-1">Escríbenos</h4>
                <p className="text-slate-600 font-bold text-sm sm:text-base">randaleonel@gmail.com</p>
              </div>
            </div>

            <div className="flex items-center gap-4 sm:gap-6 p-4 sm:p-6 bg-slate-50 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-100 group transition-all duration-300 hover:shadow-lg hover:bg-white">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-green-500 text-white rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg shadow-green-500/20 group-hover:scale-110 transition-transform shrink-0">
              ...
              <div>
                <h4 className="font-black text-slate-900 uppercase text-[10px] tracking-widest mb-1">WhatsApp</h4>
                <p className="text-slate-600 font-bold text-sm sm:text-base">+52 729 685 3914</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative group">
          <div className="absolute -inset-1 bg-gradient-to-r from-amber-500 to-amber-700 rounded-[2.5rem] sm:rounded-[3rem] blur opacity-10 group-hover:opacity-20 transition-opacity"></div>
          <div className="relative bg-white p-6 sm:p-10 md:p-16 rounded-[2.5rem] sm:rounded-[3rem] border border-slate-100 shadow-2xl">
            {status === 'success' ? (
              <div className="h-full min-h-[350px] sm:min-h-[400px] flex flex-col items-center justify-center text-center animate-in zoom-in duration-500">
                <div className="w-20 h-20 sm:w-24 sm:h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-6 sm:mb-8 shadow-inner">
                  <svg className="w-10 h-10 sm:w-12 sm:h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                </div>
                <h3 className="text-2xl sm:text-3xl font-black text-slate-900 mb-4">¡Mensaje Recibido!</h3>
                <p className="text-slate-500 text-base sm:text-lg leading-relaxed max-w-sm">
                  Gracias por contactar a Grupo AR. Un ingeniero se pondrá en contacto contigo en las próximas 24 horas.
                </p>
                <button 
                  onClick={() => setStatus('idle')}
                  className="mt-8 sm:mt-10 bg-slate-900 text-white px-8 py-4 rounded-2xl font-bold hover:bg-slate-800 transition-all"
                >
                  Enviar otro mensaje
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className={`w-full px-6 py-3.5 sm:py-4 rounded-2xl border ${errors.name ? 'border-red-400' : 'border-slate-100'} focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none transition-all bg-slate-50/50 font-medium text-sm sm:text-base`} 
                  ...
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      onChange={handleChange} 
                      className={`w-full px-6 py-3.5 sm:py-4 rounded-2xl border ${errors.email ? 'border-red-400' : 'border-slate-100'} focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none transition-all bg-slate-50/50 font-medium text-sm sm:text-base`} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      onChange={handleChange} 
                      className={`w-full px-6 py-3.5 sm:py-4 rounded-2xl border ${errors.phone ? 'border-red-400' : 'border-slate-100'} focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none transition-all bg-slate-50/50 font-medium text-sm sm:text-base`} 
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    onChange={handleChange} 
                    className={`w-full px-6 py-3.5 sm:py-4 rounded-2xl border ${errors.message ? 'border-red-400' : 'border-slate-100'} focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none transition-all bg-slate-50/50 font-medium resize-none text-sm sm:text-base`} 
                ...
                <button 
                  ...
                  className="w-full group relative bg-slate-900 hover:bg-slate-800 text-white font-black py-4 sm:py-5 rounded-[1.5rem] sm:rounded-[2rem] transition-all overflow-hidden shadow-2xl shadow-slate-900/20 active:scale-[0.98] disabled:opacity-70"
                >
                ...
                <div className="relative flex items-center gap-4 py-1 sm:py-2">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">O también</span>
                  <div className="h-px bg-slate-100 flex-1"></div>
                </div>
                ...
                <a 
                  ...
                  className="w-full flex items-center justify-center gap-3 bg-green-500 hover:bg-green-600 text-white font-black py-4 sm:py-5 rounded-[1.5rem] sm:rounded-[2rem] transition-all shadow-xl shadow-green-500/20 active:scale-[0.98] text-sm sm:text-base"
                >
                  <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
